...
typedef enum {//---set the things might appear in the cell(use enum is better than int,althought it got limit, but is more clear)
    EMPTY,PLAYER,PIECES,METEOR,SHIELD//---The space cell,the player itself, and the pieces, and now?we got meteor and shield
} CellType;

typedef struct {
    CellType type;
} Cell;//----to make sure each cell only contain one object, empty, player,meteor, shield or rubbish
    ...
    for (int i = 0; i < MAP_SIZE; i++) {//--for x range
        for (int j = 0; j < MAP_SIZE; j++) {//--for i range
            switch (map[i][j].type) {//---allow the empty to change type to player and pieces
                ...
                case SHIELD: printf("o"); break;//--so o is equal to shield
            }
        }
        printf("\n");
    }
...
//----------for summon METEOR
void placeMeteors(int count) {//--set the pieces of the meteors equal to count(again)
    int placed = 0;//---defaukt set = 0
    while (placed < count) {//---if the meteors placed is lower than count, keep placing
        int x = rand() % MAP_SIZE;//--at random range of x
        int y = rand() % MAP_SIZE;//---at random range of y
        if (map[y][x].type == EMPTY) {//---to make sure the original location is space so we can put the meteor 
            map[y][x].type = METEOR;//----to check the pieces is placed
            placed++;//---keep placing until the number placed is equal to count
        }
    }
}
//-------------for summon shield
void placeshield(int count) {//--set the pieces of the shield equal to count(once again)
    int placed = 0;//---defaukt set = 0
    while (placed < count) {//---if the shield placed is lower than count, keep placing,but I set it only one because it's too powerful
        int x = rand() % MAP_SIZE;//--at random range of x
        int y = rand() % MAP_SIZE;//---to make sure the original location is space so we can put the pieces 
        if (map[y][x].type == EMPTY) {//---to make sure the original location is space so we can put the shield 
            map[y][x].type = SHIELD;
            placed++;//---keep placing until the number placed is equal to count
        }
    }
...
void playermovement(int *score, int *totalpieces, int *fuel, int *protected){//---still trying
char move;
...
if (newX < 0 || newX >= MAP_SIZE || newY < 0 || newY >= MAP_SIZE) {//--if the player try to move outside the page
    ...
    (*fuel)--;//---fuel decrease after every success movement
    printf("Remaining fuel: %d\n", *fuel);//---print the remaining fuel
    if (*fuel <= 0) {
        printf("Unfortunately, you ran out of fuel, now you'll be missing into the deep space\n");//---player lose the game if the fuel is = 0
        exit(0);
    }
if (map[newY][newX].type == PIECES) {//----if the pieces is touched by the player, add the score and show how many scores you still need 
     ...
     if (map[newY][newX].type == METEOR) {//----if the meteor is touched by the player, the fuel decrease 
         if (*protected){
             ...
             *protected-=1;
                if (*protected == 0) {
        printf("⚠️ Your shield is broken! You're vulnerable now!\n");
    }
         } else {
         (*fuel)-=5; 
    ...
    //-------special item, shield
    if (map[newY][newX].type == SHIELD) {//----if the player got the Shield, they got immune to meteors
         *protected = 5;
        printf("Now, you got protected, now immune to meteor in a short time!\n");//--player got strong because of the shield
    }
...
int main() {
    ...
    int totalshield = 1;//--set the amount of shield
    int protected = 0;
