    ...
    for (int i = 0; i < MAP_SIZE; i++) {//--for x range
        for (int j = 0; j < MAP_SIZE; j++) {//--for i range
            switch (map[i][j].type) {//---allow the empty to change type to player and pieces
                ...
                case PIECES: printf("E "); break;//---so E is equal to pieces
            }
        }
        printf("\n");
    }
...
int main() {
    ...
    int fuel = 10;//---set the "steps the user allow to move"

printf("Please select a difficulty:\n");//---ask user to enter the value to decide which mode they want to 
printf("1. Easy (5 pieces)\n");//--tell the user the easy mode is equal to 5 pieces
printf("2. Normal (10 pieces)\n");;//--tell the user the normal mode is equal to 10 pieces
printf("3. Hard (30 pieces)\n");;//--tell the user the hard mode is equal to 30 pieces
